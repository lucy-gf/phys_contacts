...
## function to calculate age-assortativity
...
fcn_assortativity <- function(
  ...
  pop <- pop %>% select('p_var','proportion') 
  ...
  matrix_rename <- matrix_rename %>% rename(n = value)
...
eth_age_sex <- suppressWarnings(data.table(read_xlsx(file.path("data","ethnicgroupagesex11.xlsx"), sheet = 6, skip = 3)))
